...
export function parseDateFromString(date) {
    if (date === 'today' || !date)
        return ['day', dayjs(new Date())];
    if (date.endsWith('day') || date.endsWith('days')) {
        const [d] = date.split('day');
        const days = Number.parseInt(d);
        return [
            'day',
            dayjs(new Date()).add(typeof days === 'number' && !Number.isNaN(days) ? days : 1, 'day'),
        ];
    }
    const split = date
        .split('-')
...
export function whereDate(date) {
    let where = {
        _gte: undefined,
        _lt: undefined,
    };
...
export function whereArray(arr) {
    if (!arr)
        return undefined;
