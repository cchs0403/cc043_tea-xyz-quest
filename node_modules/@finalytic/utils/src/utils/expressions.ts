...
export const expression = {
  toString(value: ExpressionTree, separator = ' '): string {
    return value
      .map((expr) => {
        switch (expr.kind) {
          case 'number':
  ...
  toTree(value: string): ExpressionTree {
    function parseValue(
    ...
    ): { tree: ExpressionTree; returnPosition: number } {
      ...
      const checkPreviousNode = (
        ...
        allowedTypes: ExpressionNode['kind'][]
      ) => {
        if (previousNode && !allowedTypes.includes(previousNode.kind)) {
          throw new Error(
            `Incorrect previous node of type ${previousNode.kind}.`
          );
        }
      };
      ...
      for (let i = 0; i < arr.length; i++) {
        ...
        if (operatorRegex.test(char)) {
          ...
          if (
            previousNode?.kind === 'operator' &&
            ['*', '/', '%'].includes(char)
          ) {
            throw new Error('Incorrect operator chain');
          }

          const node: ExpressionNode = { kind: 'operator', type: char as any };
          tree.push(node);
          continue;
        }
        ...
        if (char === '(') {
          ...
          const { tree: childTree, returnPosition } = parseValue(
            expression,
            i + 1,
            { kind: 'group', value: [], type: 'parenthesis' }
          );
          const node: ExpressionNode = {
            kind: 'group',
            type: 'parenthesis',
        ...
        if (letterRegex.test(char)) {
          ...
          if (!(defaultFunctions as any)[func]) {
            throw Error(`Function '${temp}' does not exist.`);
          }
          const { tree: childTree, returnPosition } = parseValue(
            expression,
            i + nextPos + 1,
            { kind: 'func', value: [], type: func }
          );
  ...
  execute(
    ...
    const explain: string[] = context?.explain ? [] : undefined;
    const functions = {
      ...defaultFunctions,
      ...(context.functions || {}),
    };
    const getData = context.getData || (() => undefined);
    function evaluate(node: ExpressionNode, level: number) {
      if (node.kind === 'group' || node.kind === 'func')
        return evaluateTree(node.value, node, level + 1);
      if (node.kind === 'field') {
        const data = getData(node.value);
        if (data === undefined) throw new Error(`No data for ${node.value}`);
        return data;
      }
    ...
    function evaluateTree(
      ...
      if (explain)
        explain.push(
          `Eval: ${t.map((x) => `[${Object.values(x)}]`).join(', ')}`
        );
      if (level > 10) throw new Error(`Max recursion depth reached (${level})`);
      let sum = 0;
      const tree = [...t];

      for (let i = 0; i < tree.length; i++) {
        const node = tree[i];
        const previousNode: ExpressionNode | undefined =
          i > 0 ? tree[i - 1] : undefined;
        ...
        if (explain) {
          const part = previousNode
            ...
            : '';
          explain.push(
            `${new Array(level).join(' ')}${part}current [${Object.values(
              node
            )}], sum ${sum}`
          );
        }
        ...
        {
          const nextNode: ExpressionNode | undefined = tree[i + 1];
          ...
          if (node?.kind === 'operator' && previousNode?.kind === 'operator') {
            ...
            // disallow 1 * * 1
            if (
              (isBasic(previousNode) && isMultiply(node)) ||
              (isMultiply(previousNode) && isMultiply(node))
            )
              throw new Error(
                `Incorrect operator chain ${previousNode.type}, ${node.type}`
              );
            if (isMultiply(previousNode) && isBasic(node)) {
              // Find next non-operator index
              const nextOperandIndex =
                tree.findIndex(
                  (x, index) => x.kind !== 'operator' && index > i
                ) + 1;
          ...
          // e.g. 1 * 1 + 2 => (1 * 1) + 2
          if (
            nextNode?.kind === 'operator' &&
            parentNode?.kind !== 'group' &&
            isMultiplyMod(nextNode)
          ) {
            // Find next non-operator index
            const nextOperandIndex =
              tree.findIndex((x, index) => x.kind !== 'operator' && index > i) +
              1;
            // Cut all elements between i and nextOperandIndex and insert them into a group
        ...
        if (node.kind !== 'operator') {
          if (!previousNode) {
            ...
            if (previousNode.kind === 'operator') {
              ...
              fn =
                (functions as any)[parentNode.type.toLowerCase()] ||
                (functions as any)[parentNode.type.toUpperCase()];
              if (!fn) {
            ...
            if (!fn) {
              throw new Error(
                `No operator or function found for node ${node.kind}/${node.value}`
              );
            }
            sum = fn(sum, current);
          }
  ...
  async executeAsync(
    ...
    const getData = context.getData || (() => undefined);
    async function evaluateAsync(node: ExpressionNode, level: number) {
      if (node.kind === 'group' || node.kind === 'func')
    ...
    async function evaluateTreeAsync(
      t: ExpressionTree,
      parentNode?: ExpressionNode,
      level = 0
    ) {
...
function isMultiplyMod(node: ExpressionNode) {
  return (
    node.kind === 'operator' &&
...
function isMultiply(node: ExpressionNode) {
  return node.kind === 'operator' && (node.type === '*' || node.type === '/');
}

function isBasic(node: ExpressionNode) {
  return node.kind === 'operator' && (node.type === '+' || node.type === '-');
}
