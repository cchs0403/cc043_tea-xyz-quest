...
const isUnderByteLimit = (buffer: LogLine[]) =>
  utils.jsonByteSize(buffer) < FLUSH_BYTE_LIMIT;

const defaultOptions = ensure<LogOptions>({
...
let defaultOptionsOverwrite: Partial<LogOptions> | undefined = undefined;
export function setDefaultLogOptions(options: Partial<LogOptions>) {
...
export class Logger extends Log {
  ...
  constructor(
    private readonly options: LogOptions = {} as any,
    initialContext?: Context
  ) {
    const context = initialContext || options.defaultContext || {};
    super(
      context,
      (message: any, context?: Context, level: LogLevel = 'log') => {
        if (
          `${message}`.includes(
  ...
  captureMessage = ({ level = 'log', message, context = {} }: LogMessage) => {
  ...
  generateLogLine = ({
    ...
    if (opts.type === 'pretty') {
      ...
      utils.originalConsole[level](
        ...[
          `[${this.getOptions().hostname}/${
            this.getOptions().app
          }] [${new Date().toISOString()} +${this.lastTime - lastTime}ms]`,
          ...
          .map((x) =>
            typeof x === 'object' ? utils.stringify(x, null as any, 2) : x
          )
      );
    } else {
      // run the beforeSend hooks
      const data: LogMessage = (
        opts.hooks || { beforeSend: [] }
      ).beforeSend.reduce(
        (acc: LogMessage, fn: any) => (acc == null ? null : fn(acc)),
        {
          level,
          message,
          context,
      ...
      const line: Omit<LogLine, 'line'> = {
        timestamp: Math.floor(Date.now() / 1000),
        app: opts.app,
        level: data.level,
        meta: context || {},
      };
      if (error) line.meta.error = error;
      ...
      if (opts.type === 'stdout')
        utils.originalConsole.log(
          JSON.stringify({ message: logMessage, ...line })
        );
      else this.process({ line: logMessage, ...line });
    }
  };
  ...
  splitAndFlush = async (logLines: LogLine[]) => {
    const lines: LogLine[] = [...logLines];
    const half: number = Math.floor(lines.length / 2);
    const lines2: LogLine[] = lines.splice(half);
    this.flush(lines);
    this.flush(lines2);
  };

  flush = async (lines?: LogLine[]) => {
    if (!lines) {
      lines = [...this.buffer];
      this.buffer = [];
    }
  ...
  process = async (lines: LogLine | LogLine[]) => {
  ...
  send = async (lines: LogLine[]) => {
    ...
    if (opts.tags?.length) params.push(['tags', opts.tags as any]);
...
const internalErrorLogger = (message: string) => {
